...
async function findAccount(email, selectFields = "") {
  ...
  account = await HostAdmin.findOne({
    email: email.toLowerCase().trim(),
  }).select(selectFields);
  ...
  try {
    ...
    if (modelType === "user") {
      if (!account.isActive) {
        return res
          .status(403)
          .json({ error: "Account is inactive. Contact administrator." });
      }
      if (
        account.accountLockedUntil &&
        account.accountLockedUntil > new Date()
      ) {
        return res
          .status(423)
    ...
    if (!account.password) {
      return res
        .status(400)
        .json({ error: "This account uses OTP login. Use the OTP tab." });
    }
...
router.post("/resend-otp", async (req, res) => {
  try {
    ...
    if (!account) {
      return res.json({
        success: true,
        message: "If email exists, OTP has been resent",
      });
    }
