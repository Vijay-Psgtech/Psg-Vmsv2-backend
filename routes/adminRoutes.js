...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!userId || !gateId) {
        return res.status(400).json({ error: "userId and gateId required" });
      }
...
router.post(
  ...
  async (req, res) => {
    try {
      const { name, email, phone, password, role, isActive, department } =
        req.body;
      if (!name || !email || !role) {
        return res
          .status(400)
  ...
  async (req, res) => {
    try {
      ...
      if (existingDepartment) {
        return res
          .status(400)
          .json({ error: "Department code already in use" });
      }
...
// fetch only admin and active users as hosts
